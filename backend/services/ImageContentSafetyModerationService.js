...
class ImageContentSafetyModerationService {
  ...
  static async inspectImage(imageFilePath, originalFileName = '') {
    // Ei function image file analyze kore safety verdict return kore:
    // { isSafe: boolean, flagType: 'clean' | 'nudity' | 'violence' | 'inappropriate', reason: string, reasonBn: string }

    try {
      // 1. Initial layer: Check if file exists
      ...
      // 2. Keyword & Heuristic Layer
      const baseName = (originalFileName || path.basename(imageFilePath)).toLowerCase();
      for (const kw of NSFW_KEYWORDS) {
        // Regex word boundary or token match to avoid accidental matches like 'assessment' -> 'ass'
        ...
        if (regex.test(baseName)) {
          ...
          return {
            isSafe: false,
            flagType: 'nudity',
            reason: 'Inappropriate or adult content detected in uploaded image filename/content. Adult and inappropriate materials are strictly forbidden.',
            reasonBn: 'আপলোড করা ছবিতে নগ্নতা বা অনৈতিক কন্টেন্ট সনাক্ত হয়েছে। জানা বাংলায় নগ্ন বা আপত্তিকর ছবি আপলোড সম্পূর্ণ নিষিদ্ধ।'
          };
        }
      }

      // 3. Gemini Vision AI Safety Inspection
...
Respond ONLY with valid raw JSON adhering strictly to this schema:
{
  ...
  "reasonBn": "বাংলায় কারণ (যথা: নগ্নতা বা আপত্তিকর ছবি অনুমোদিত নয়)"
          ...
          // Check safety ratings if available in candidates
          ...
          // If Gemini throws a safety block error
