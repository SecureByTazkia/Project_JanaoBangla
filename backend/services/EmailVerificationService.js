...
// Gmail SMTP diye Nodemailer use kora hobe
// ==========================================
...
async function sendEmailVerificationOTP(userEmail, userName, otp) {
  // Email configured ache ki na check kora hocche
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASSWORD ||
      process.env.EMAIL_USER.includes('your_gmail')) {
    // Development mode e email not configured thakle console e dekhabe
    console.log(`📧 [DEV MODE] Email verification OTP for ${userEmail}: ${otp}`);
    console.log('   Configure EMAIL_USER and EMAIL_PASSWORD in .env for real emails');
  ...
  try {
    ...
    // Email er HTML content define kora hocche
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    // Email fail holeo registration fail hobe na — dev log e dekhabe
    console.log(`📧 [FALLBACK] OTP for ${userEmail}: ${otp}`);
...
async function sendPasswordResetEmail(userEmail, userName, resetToken) {
  // Reset URL frontend URL theke banano hocche
  const resetUrl = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/reset-password?token=${resetToken}`;

  // Email configured ache ki na check kora hocche
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASSWORD ||
      process.env.EMAIL_USER.includes('your_gmail')) {
    console.log(`📧 [DEV MODE] Password reset for ${userEmail}`);
  ...
  try {
    ...
    console.log(`📧 [FALLBACK] Reset URL for ${userEmail}: ${resetUrl}`);
    return { success: false, error: error.message };
  }
}

module.exports = { sendEmailVerificationOTP, sendPasswordResetEmail };
